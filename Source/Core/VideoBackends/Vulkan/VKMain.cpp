...
std::unique_ptr<AbstractGfx> VideoBackend::CreateGfx()
{
  ...
  return gfx;  // std::make_unique<VKGfx>(this, std::move(vulkan_context),
               // std::move(command_buffer_mgr), std::move(object_cache), std::move(swap_chain),
               // std::move(state_tracker), m_wsi.render_surface_scale);
}
