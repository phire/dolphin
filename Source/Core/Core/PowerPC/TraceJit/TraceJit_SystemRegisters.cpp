...
void TraceJit::mtspr(UGeckoInstruction inst)
{
	INSTRUCTION_START
	...
	case SPR_GQR0 + 7:
		...
		// This will create a little bit of block churn, but hopefully not too bad.
		{
			...
			ABI_CallFunctionC(ProtectFunction(&TraceJit::DestroyBlocksWithFlag, 1), (u32)BLOCK_USE_GQR0 << gqr);
...
void TraceJit::mfspr(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
	u32 iIndex = (inst.SPRU << 5) | (inst.SPRL & 0x1F);
...
void TraceJit::mfmsr(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
	//Privileged?
...
void TraceJit::mftb(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
	mfspr(inst);
...
void TraceJit::mfcr(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
	// USES_CR
...
void TraceJit::mtcrf(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
...
void TraceJit::mcrf(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
...
void TraceJit::mcrxr(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
...
void TraceJit::crXXX(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITSystemRegistersOff)
