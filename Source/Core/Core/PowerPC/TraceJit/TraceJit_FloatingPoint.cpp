...
void TraceJit::fp_tri_op(int d, int a, int b, bool reversible, bool single, void (XEmitter::*op)(Gen::X64Reg, Gen::OpArg))
{
	fpr.Lock(d, a, b);
	if (d == a)
	{
...
void TraceJit::fp_arith(UGeckoInstruction inst)
{
	INSTRUCTION_START
...
void TraceJit::frsqrtex(UGeckoInstruction inst)
{
       INSTRUCTION_START
       JITDISABLE(bJITFloatingPointOff)
       int d = inst.FD;
...
void TraceJit::fmaddXX(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITFloatingPointOff)
	if (inst.Rc) {
...
void TraceJit::fsign(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITFloatingPointOff)
	if (inst.Rc) {
...
void TraceJit::fmrx(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITFloatingPointOff)
	if (inst.Rc) {
...
void TraceJit::fcmpx(UGeckoInstruction inst)
{
	INSTRUCTION_START
	JITDISABLE(bJITFloatingPointOff)
