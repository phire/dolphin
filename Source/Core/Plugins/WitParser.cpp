...
std::vector<Wit::PackageItem> parse_wit() {
    ...
    if (!result) {
        auto [error_message, remaining_input] = result.error();
        fmt::print(stderr, "Error parsing dolphin.wit\n");
        size_t error_pos = dolphin_wit.size() - remaining_input.size();
        size_t error_line_num = std::count(dolphin_wit.begin(), dolphin_wit.begin() + error_pos, '\n') + 1;
        size_t start_of_line = dolphin_wit.rfind('\n', error_pos);
        if (start_of_line == std::string_view::npos)
            start_of_line = 0;
        else
            start_of_line += 1;
        size_t end_of_line = dolphin_wit.find('\n', error_pos);
